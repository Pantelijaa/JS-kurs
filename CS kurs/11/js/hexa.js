        ...
        <div class="single-post" data-post_id="${post.id}">
            ...
            <div class="post-actions">
                ...
                <div>
                    ...
                    <button onclick="dislikePost(this)" class="dislikePostJS dislike-btn"><span>${post.dislikes}</span> dislikes</button>
        ...
        async function getPostUser() {
            ...
            if (comments.length > 0) {
                comments.forEach(comment => {
                    comments_html += `<div class="single-comment">${comment.content} - by <b>${comment.username}</b></div>`;
                });
            }
...
const commentPostSubmit = e => {
    ...
    main_post_el.querySelector('.post-comments').innerHTML += `<div class="single-comment">${comment_value} - by <b>${username}</b></div>`;
...
const dislikePost = btn => {
    let main_post_el = btn.closest('.single-post');
    let post_id = btn.closest('.single-post').getAttribute('data-post_id');
    let number_of_dislikes = parseInt(btn.querySelector('span').innerText);

    btn.querySelector('span').innerText = number_of_dislikes + 1;
    btn.setAttribute('disabled', 'true');

    let post = new Post();
    post.dislike(post_id, number_of_dislikes + 1);
}
